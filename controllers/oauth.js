...
exports.getNewYorkTimes = function(req, res, next) {
  ...
  var url = 'http://oauth.nytimes.com/svc/books/v2/lists?' + query;
...
exports.getSteam = function(req, res, next) {
  ...
  async.parallel({
    playerAchievements: function(done) {
      ...
      request.get({ url: 'http://oauth.steampowered.com/ISteamUserStats/GetPlayerAchievements/v0001/?' + qs, json: true }, function(error, request, body) {
    ...
    playerSummaries: function(done) {
      ...
      var qs = querystring.stringify(query);
      request.get({ url: 'http://oauth.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?' + qs, json: true }, function(err, request, body) {
        if (request.statusCode === 401) {
          return done(new Error('Missing or Invalid Steam API Key'));
        }
        done(err, body);
    ...
    ownedGames: function(done) {
      ...
      var qs = querystring.stringify(query);
      request.get({ url: 'http://oauth.steampowered.com/IPlayerService/GetOwnedGames/v0001/?' + qs, json: true }, function(err, request, body) {
        if (request.statusCode === 401) {
          return done(new Error('Missing or Invalid Steam API Key'));
        }
        done(err, body);
...
exports.postTwilio = function(req, res, next) {
  ...
  if (errors) {
    req.flash('errors', errors);
    return res.redirect('/oauth/twilio');
  }
...
exports.getVenmo = function(req, res, next) {
  ...
  async.parallel({
    getProfile: function(done) {
      request.get({ url: 'https://oauth.venmo.com/v1/me?' + query, json: true }, function(err, request, body) {
        done(err, body);
      });
    },
    getRecentPayments: function(done) {
      request.get({ url: 'https://oauth.venmo.com/v1/payments?' + query, json: true }, function(err, request, body) {
        done(err, body);
      });
    }
  },
...
exports.postVenmo = function(req, res, next) {
  ...
  if (errors) {
    req.flash('errors', errors);
    return res.redirect('/oauth/venmo');
  }
  ...
  request.post('https://oauth.venmo.com/v1/payments', { form: formData }, function(err, request, body) {
    if (err) {
      return next(err);
    }
    ...
    bitgo.wallets().get({ id: walletId }, function(err, wallet) {
      wallet.sendCoins({
        ...
        if (err) {
          ...
          return res.redirect('/oauth/bitgo');
        }
